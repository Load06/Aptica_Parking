...
import { Bell, Map, Key, RotateCw, Plus } from 'lucide-react';
...
export function HomeScreen({ onLiberate }: HomeProps) {
  ...
  useEffect(() => {
    ...
    api.get<Liberation[]>(`/plazas/availability?date=${d}`).then(r => {
      setTodayLibs(r.data);
      const mine = r.data.find(l => l.reservation?.userId === user?.id);
    ...
    <div className="px-5 pt-4 pb-4">
      ...
      {user?.role === 'floating' && myReservation && (
        <Card className="rounded-2xl p-5 mb-5">
          ...
          <Button variant="secondary" size="md" fullWidth className="mt-4" onClick={() => navigate('/mapa')}>
      ...
      <Card className="rounded-xl overflow-hidden">
        ...
        {todayLibs.slice(0, 5).map((lib, i) => (
          ...
          >
            <Avatar name={lib.plaza?.owner?.name ?? '?'} color={lib.plaza?.owner?.avatarColor ?? '#6A1873'} size={38} />
            <div className="flex-1 min-w-0">
              <p className="text-[14px] font-semibold text-ink truncate">
                <span className="font-bold">{lib.plaza?.owner?.name}</span>
            ...
            {user?.role === 'floating' && !lib.reservation && (
